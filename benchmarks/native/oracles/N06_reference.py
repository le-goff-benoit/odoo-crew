...
class LabRegister(models.Model):
    ...
    def action_repair(self):
        for index, record in enumerate(self.filtered(lambda row: row.state == 'draft' and row.company_id == self.env.company).sorted(lambda row: (row.date_document, row.id)), 1):
            record.write({'sequence': index * 100, 'snapshot_total': sum(line.quantity * line.price for line in record.line_ids if not line.cancelled)})
        return True
