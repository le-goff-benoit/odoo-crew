...
## Autorisations et entrées RPC

Le contexte RPC est fourni par l'appelant : un booléen `context` ne peut jamais
autoriser une écriture protégée ou désactiver un contrôle de droits. Vérifier les
habilitations avant une écriture interne, sans exposer ce contournement par
create/write. Les `default_*` du contexte alimentent aussi les champs absents
de create : les valeurs protégées doivent rester sûres après application de ces
défauts. Tester le résultat métier avec un utilisateur limité ; une valeur
refusée ou ignorée doit laisser l'état protégé inchangé.

À la création, retirer `approved` de `vals` ne neutralise pas `default_approved` :
`super().create()` applique ensuite les valeurs par défaut. Fixer explicitement
