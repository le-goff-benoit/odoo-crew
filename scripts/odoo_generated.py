...
from odoo_models import codex_agent
...
def expected_outputs(root: Path, destination: Path) -> dict[Path, str]:
    ...
    for declaration in declarations:
        ...
        body += "\n" + (root / "roles/model-policy.md").read_text(encoding="utf-8")
        marker = (
        ...
        if kind == "emit_command":
            ...
            outputs[destination / '.codex/agents' / f'{slug}.toml'] = codex_agent(slug, desc, body)
        else:
            short, desc = args
...
def emit_native_agents(destination):
    for target, content in expected_outputs(ROOT, destination).items():
        if target.suffix != '.toml' or target.parent.name != 'agents':
            continue
        if target.is_symlink():
            raise ValueError('Lien de profil à conserver : ' + str(target))
        if target.exists():
            previous = target.read_text()
            modern = previous.startswith('# Generated by odoo19-agents/build.sh;')
            legacy = ('<!-- Généré par ~/.odoo19-agents/build.sh — ne pas éditer ici.' in previous
                      and 'Source : ~/.odoo19-agents/roles/' in previous)
            if not modern and not legacy:
                raise ValueError('Profil non généré à conserver : ' + str(target))
            if legacy:
                backup = destination / '.odoo-agents-backups/native-agents'
                backup.mkdir(parents=True, exist_ok=True)
                saved = backup / (target.stem + '-' + hashlib.sha256(previous.encode()).hexdigest()[:12] + '.toml')
                if not saved.exists():
...
if __name__ == "__main__":
    if len(sys.argv) == 3 and sys.argv[2] == '--emit-codex-agents':
        emit_native_agents(Path(sys.argv[1]))
        sys.exit(0)
    if len(sys.argv) != 2:
