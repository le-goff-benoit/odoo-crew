...
def snapshot_candidate(source, target):
    ...
    for name in ('roles', 'scripts', 'docs', 'workflows', 'tests', 'benchmarks'):
        if any(p.is_symlink() for p in (source / name).rglob('*')
               if not set(p.relative_to(source / name).parts) & {'__pycache__', 'quality-lab', 'runs'} and not p.name.endswith('.pyc')):
            raise ValueError('lien symbolique interdit dans le snapshot : ' + name)
...
class SourceServer(socketserver.UnixStreamServer):
    ...
    def read_source(self, name):
        if name.startswith('~/'):
            name = str(HOME_PATH) + name[1:]
        text = self.source_path(name).read_text()
...
def audit_read_outputs(raw, reads, texts):
    """Verify full instrumented content in native tool results, not just reader counts."""
    outputs = []
        ...
        if event.get('type') == 'user':
            for item in event.get('message', {}).get('content', []):
                if isinstance(item, dict) and item.get('type') == 'tool_result':
                    ...
                    outputs.append(value if isinstance(value, str) else '\n'.join(
                        part.get('text', '') for part in value if isinstance(part, dict)))
    return [{'path': row['path'], 'source_hash_unchanged': digest(texts[row['path']].encode()) == row['sha256'],
             'full_text_in_native_tool_result': any(texts[row['path']].strip() in out for out in outputs),
             'characters_instrumented': len(texts[row['path']])} for row in reads]
...
def run_trial(folder, pack, generated, case, request, expected, provider, config, timeout):
    ...
    audit_result = audit_read_outputs(folder / 'raw.jsonl', reads, {name: server.source_path(name).read_text() for name in observed})
    native_outputs_complete = all(r['source_hash_unchanged'] and r['full_text_in_native_tool_result'] for r in audit_result)
    atomic_json(folder / 'native-output-audit.json', audit_result)
    if not native_outputs_complete:
        problems.append('texte du lecteur non retrouvé intégralement dans les sorties natives')
    ...
    state = {'provider': provider, 'case': case.name, 'config': config, 'seconds': round(time.monotonic()-started, 3),
             ...
             'reading': {'native_outputs_complete': native_outputs_complete, 'calls': len(reads), 'total_characters_returned': sum(r['characters'] for r in reads),
...
def run(output, corpus, candidate, baseline, timeout):
    if output.exists() or not 1 <= timeout <= 600:
        raise ValueError('nouveau dossier requis et timeout 1–600 s')
    ...
    if cases['cases'] != ['positive', 'holdout']:
        raise ValueError('deux cas prescrits requis pour budget de 8 appels')
