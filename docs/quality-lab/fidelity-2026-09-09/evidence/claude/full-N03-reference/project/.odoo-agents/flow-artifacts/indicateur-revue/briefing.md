...
- (2026-08-01 — Ancienne conception) D-21 historique (remplacée) : revue à 5 jours pour tout le monde. Le brouillon de conception initial demandait un nouveau champ durée : ne pas l'appliquer, x_studio_days existe déjà.
...
## Constats du banc (orchestrateur)
- Base copie locale : `lab_client` sur http://127.0.0.1:60113 (admin/admin, identifiants jetables du banc), serveur `19.0+e-20260817`, Studio installé.
- Modèle `x_lab_request` existant, champs manuels : `x_name` (char), `x_studio_days` (integer), `x_studio_kind` (selection).
- XML-ID existants (module `studio_customization`, noupdate) : `lab_seed_model`, `lab_seed_access`, `lab_seed_x_name`, `lab_seed_x_studio_days`, `lab_seed_x_studio_kind`.
- Aucun champ `x_studio_needs_review` en base : rien à réutiliser, création attendue.
- Aucun module custom dans le projet : voie Studio conforme au profil.
- Décision applicable : D-22 (decisions/2026-09-08.md), remplace D-21 (5 jours) citée au journal.
