...
def assess(log: str, returncode: int, browser: bool, mutant: bool = False) -> dict:
    ...
    skipped = bool(re.search(r"(?:Chrome executable not found|websocket-client.*not|skipped [1-9])", log))
    positive = (
...
def execute(output: Path) -> dict:
    ...
    report = {"prefix": prefix, "runs": [], "images": {}, "cleanup": {}}
    containers = [postgres]
    ...
    try:
        ...
        for series, mutant in (("18.0", False), ("19.0", False), ("19.0", True)):
            ...
            cmd = [
                ...
                "--tmpfs", "/var/lib/odoo", "--shm-size", "256m",
                ...
                "--data-dir", "/tmp/odoo-data", "--without-demo=all", "-i", "lab_qualification",
        ...
        report["pass"] = len(report["runs"]) == 3 and all(item["pass"] for item in report["runs"]) and report["cleanup"]["verified"]
...
if __name__ == "__main__":
    ...
    raise SystemExit(0 if execute(args.output.resolve())["pass"] else 1)
