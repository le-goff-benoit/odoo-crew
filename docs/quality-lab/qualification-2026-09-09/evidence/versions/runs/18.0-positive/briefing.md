...
- **Série** : **18.0** (origine : explicite) · sources `/home/blegoff/odoo-sources/18.0` + `18.0-enterprise`
- **Racine** : `/tmp/odoo-qualification-versions-20260909/runs/18.0-positive/project`
- **Release** : aucune release ouverte — s'ouvre après le verdict de l'analyste ou du support (`odoo-release.sh open <projet> "<titre>"`)
- **Instances déclarées** : Aucun environnement déclaré pour project. → odoo_instance.py add /tmp/odoo-qualification-versions-20260909/runs/18.0-positive/project
- **Hors ligne** : bases locales et versions déployées non interrogées.

## Formes attendues en 18.0

**Différences avec le guide 19.0** : `_sql_constraints` (et non `models.Constraint`) · listes de domaine (pas d'objet `Domain`) · `category_id` sur les groupes (pas de `privilege`) · `res.users.groups_id` (et non `group_ids`) · `hr.contract` existe encore · `ir.model.access.csv` + `ir.rule`
Formes en vigueur : `invisible="expr"` (plus d'`attrs`/`states`) · `_compute_display_name` (plus de `name_get`) · `<list>` (plus de `<tree>`) · `<chatter/>` · `self.env._()` disponible · `@api.readonly` disponible

## Leçons du dispositif applicables en 18.0 (LESSONS.md)
