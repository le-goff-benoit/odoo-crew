...
for line in raw.splitlines():
 ...
 elif typ=='event_msg' and payload.get('type')=='task_complete':x['payload']={k:v for k,v in payload.items() if k in ('type','turn_id','started_at','completed_at','duration_ms','time_to_first_token_ms')}
 elif typ=='response_item':
  keep=payload.get('type') in ('function_call','function_call_output','custom_tool_call','custom_tool_call_output')
  keep=keep or (payload.get('type')=='message' and payload.get('role')=='assistant' and (payload.get('channel')=='final' or payload.get('phase')=='final_answer'))
  if not keep:continue
 else:continue
 rows.append(x)
a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(''.join(json.dumps(x,ensure_ascii=False)+'\n' for x in rows))
manifest={'agent':a.agent,'thread_id':meta['id'],'source_snapshot_sha256':hashlib.sha256(raw).hexdigest(),'export_sha256':hashlib.sha256(a.output.read_bytes()).hexdigest(),'records':len(rows),'policy':'Allowlisted native session records: agent provenance, model/effort, tool calls/results, final answer and usage. All system/developer/user messages, analysis and other events omitted except task_complete timing metadata (last_agent_message excluded). Source is live append-only; hash identifies bytes at export time.'}
