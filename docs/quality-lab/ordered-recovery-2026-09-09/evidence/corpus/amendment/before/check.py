...
def check(folder):
    ...
    if case=='O03':
        ...
        if case=='O02':
            ...
            record('positive_no_retry_or_extra_gate',all(e.get('node')=='journal_task' and e.get('outcome')=='done' for e in new) and len(new)==1)
    record('no_orphan_flow_claim',not current.get('claims'))
    registry=Path(current['resource_registry']);record('no_orphan_resource_claim',not json.loads(registry.read_text()).get('claims'))
