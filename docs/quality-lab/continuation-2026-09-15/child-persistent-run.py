...
BASE=Path('/tmp/tricorder-native-child-persistent-20260915'); BASE.mkdir(exist_ok=True)
CREW=Path('/home/blegoff/.odoo19-agents')
PROBE=CREW/'tests/fixtures/orchestration_probe.py'
results=[]
for provider,model in [('codex','gpt-6-astra')]:
    root=BASE/provider
    ...
    prompt=f"Essai natif synthétique autorisé, sans base ni projet Odoo. Délègue explicitement à un sous-agent une seule vérification : exécuter python3 -c \"import time; time.sleep(2); print('preuve-enfant-ok')\". Le sous-agent ne touche aucun fichier ni état de plan. Attends son résultat avec le mécanisme natif, puis examine ce résultat avant de lancer toi-même python3 {PROBE} run {root} A. Les tâches A,B,C sont autorisées. Après A, termine le tour ; si un rappel arrive, exécute réellement B puis C avec la même commande. Conserve le modèle principal pour toi. Ne crée qu'un enfant, aucune autre action."
    args=shlex.split(launch['command'])
    if provider=='codex':
        args=[args[0],'exec',*args[1:],'--ignore-user-config','--ignore-rules','--json','--skip-git-repo-check','--sandbox','workspace-write','--model',model,'--dangerously-bypass-hook-trust','-c','model_reasoning_effort="medium"','-c','web_search="disabled"','--disable','apps','--disable','plugins','--enable','multi_agent','-']
    else:
        args+=['--print','--restricted','--strict-mcp-config','--setting-sources','','--tools','Bash,Agent','--allowedTools','Bash(python3 *),Agent','--no-session-persistence','--output-format','stream-json','--verbose','--include-hook-events','--model',model,'--effort','medium','--max-turns','12']
