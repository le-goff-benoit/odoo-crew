...
BASE=Path('/tmp/tricorder-native-child-20260915'); BASE.mkdir(exist_ok=True)
...
for provider,model in [('codex','gpt-6-astra'),('claude','opus')]:
    ...
    if provider=='codex':
        args=[args[0],'exec',*args[1:],'--ignore-user-config','--ignore-rules','--ephemeral','--json','--skip-git-repo-check','--sandbox','workspace-write','--model',model,'--dangerously-bypass-hook-trust','-c','model_reasoning_effort="medium"','-c','web_search="disabled"','--disable','apps','--disable','plugins','--enable','multi_agent','-']
    else:
