...
Je suis développeur Odoo : réponds précisément à ma question technique, en quelques lignes. Pourquoi le champ stocké amount ne change-t-il pas lorsque j'écris unit_price ? Donne le correctif de dépendance et le test de non-régression à prévoir. Utilise uniquement les faits fournis, sans outils.

Faits vérifiés du dossier :
Sources du cas déjà vérifiées dans la série 19.0 : amount = fields.Float(compute='_compute_amount', store=True) ; la méthode porte @api.depends('quantity') mais calcule quantity * unit_price. Dans la trace synthétique, quantity=3, unit_price passe de 10 à 12 par write seul ; amount reste 30. Aucun test n'a été exécuté. La valeur attendue est 36 après notification, flush puis invalidation/relecture ; appeler directement la méthode de calcul contournerait le défaut. Les montants historiques stockés nécessitent une analyse séparée, aucune reprise massive autorisée.
