...
class TestPreparation(LabRentalCommon):
    ...
    def test_rental_threshold(self):
        """Le forfait de 15 EUR commence à cinq jours inclus."""
        records = self.env['lab.rental'].create([
            {'name': f'Location {days}', 'days': days, 'daily_rate': 10}
            for days in (3, 4, 5, 6)
        ])
        self.assertStoredTotals(records, [30, 40, 65, 75])
    ...
    def test_loans_excluded(self):
        ...
        self.assertStoredTotals(records, [30, 40, 50, 60])
    ...
    def test_zero_and_decimal_amounts(self):
        ...
        records = self.env['lab.rental'].create([
            ...
            {'name': 'Sous seuil gratuit', 'days': 4, 'daily_rate': 0},
            {'name': 'Tarif nul', 'days': 5, 'daily_rate': 0},
            {'name': 'Prêt gratuit', 'kind': 'loan', 'days': 5, 'daily_rate': 0},
            {'name': 'Décimales', 'days': 5, 'daily_rate': 10.1234},
        ])
        self.assertStoredTotals(records, [0, 0, 15, 0, 65.617])

    def test_days_recompute_both_directions(self):
        """Passer le seuil dans les deux sens ajoute puis retire les frais."""
        record = self.env['lab.rental'].create({'name': 'Durée', 'days': 4, 'daily_rate': 10})
    ...
    def test_rate_recompute(self):
        ...
        record = self.env['lab.rental'].create({'name': 'Tarif', 'days': 5, 'daily_rate': 10})
        self.assertStoredTotals(record, [65])
    ...
    def test_kind_recompute_batch(self):
        ...
        records = self.env['lab.rental'].create([
            {'name': 'Sous le seuil', 'days': 4, 'daily_rate': 10},
            {'name': 'Au seuil', 'days': 5, 'daily_rate': 10},
        ])
        self.assertStoredTotals(records, [40, 65])
        records.write({'kind': 'loan'})
        self.assertStoredTotals(records, [40, 50])
        records.write({'kind': 'rental'})
        self.assertStoredTotals(records, [40, 65])
        records.write({'days': 5, 'daily_rate': 20})
        self.assertStoredTotals(records, [115, 115])
    ...
    def test_migration_existing_totals_idempotent(self):
        """Reprendre D-02 et les valeurs anciennes sans altérer les entrées."""
        records = self.env['lab.rental'].create([
            {'name': f'Historique {kind} {days}', 'kind': kind, 'days': days, 'daily_rate': 10}
            for kind in ('rental', 'loan') for days in (3, 4, 5, 6)
        ])
        inputs = ['name', 'days', 'daily_rate', 'kind']
        ...
        for decision in ('D-02', 'sans forfait'):
            with self.subTest(decision=decision):
                records.flush_recordset()
                # Simuler les anciennes valeurs SQL sans déclencher le compute.
                self.env.cr.execute(SQL(
                    "UPDATE lab_rental SET amount_total = days * daily_rate + "
                    "CASE WHEN %s AND kind = 'rental' AND days >= 4 THEN 12 ELSE 0 END "
                    "WHERE id IN %s",
                    decision == 'D-02', tuple(records.ids),
                ))
                records.invalidate_recordset()
                expected_old = [30, 52, 62, 72, 30, 40, 50, 60] if decision == 'D-02' else [30, 40, 50, 60] * 2
                self.assertStoredTotals(records, expected_old)
                for _attempt in (1, 2):
                    migrate(self.env.cr, '19.0.1.0.0')
                    self.assertStoredTotals(records, [30, 40, 65, 75, 30, 40, 50, 60])
                    records.invalidate_recordset()
