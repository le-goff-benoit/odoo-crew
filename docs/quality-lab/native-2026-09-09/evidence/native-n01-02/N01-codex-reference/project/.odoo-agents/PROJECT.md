...
## Décisions actées
D-03, actée par Alice Martin (decisions/2026-09-09.md), remplace D-02 dès la release ouverte : jours × tarif + 15 EUR uniquement pour les locations dès 5 jours inclus ; prêts exclus ; aucun arrondi supplémentaire. D-02 (12 EUR dès 4 jours) et D-01 (7 %) sont historiques, non applicables. Entrées positives ou nulles, sans écran ni facture. Release à laisser ouverte.
## Pièges connus
...
Une QA validée sur une décision remplacée reste une preuve historique : D-03 exige une nouvelle QA et une nouvelle reprise des totaux stockés D-02, y compris le retrait des frais à 4 jours.

Le recalcul stocké actualise write_date/write_uid selon Odoo 19.0 (_write_multi). Conserver les entrées métier est obligatoire ; conserver ces métadonnées ne l’est pas. Un test dans une transaction unique peut masquer leur évolution : contrôler aussi sur copie entre sessions.
