...
class LabDispatch(models.Model):
    ...
    def action_recalculate(self):
        """Recalculer les brouillons hors lignes annulées, sans toucher aux validés."""
        for record in self.filtered(lambda dispatch: dispatch.state == 'draft'):
            total = sum(line.quantity * line.price for line in record.line_ids if not line.cancelled)
