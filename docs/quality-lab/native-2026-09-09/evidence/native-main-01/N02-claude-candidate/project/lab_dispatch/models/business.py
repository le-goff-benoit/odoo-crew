...
class LabDispatch(models.Model):
    ...
    def action_recalculate(self):
        """Recompute the stored total of draft dispatches only (decision D-12).

        Validated dispatches are definitively frozen: they are filtered out
        *before* any assignment, so that not even an identical value is written
        on them. Cancelled lines are excluded from the total.
        """
