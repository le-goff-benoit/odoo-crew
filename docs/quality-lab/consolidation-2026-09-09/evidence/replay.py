...
if sys.argv[1].startswith('held-'):
    ...
    previous = Path('/tmp/odoo-delegation-20260909/N04-claude-delegated/after-0')
...
with (folder / 'raw.jsonl').open('w') as out, (folder / 'stderr.log').open('w') as err:
    proc = subprocess.Popen(sandbox(home, work, pack, provider='claude') + native_command('claude', config), stdin=subprocess.PIPE, stdout=out, stderr=err, text=True, env=provider_environment(), start_new_session=True)
    status = 'executed'
