...
folder = BASE / 'rpc-runtime-calibrated'; folder.mkdir()
...
try:
    ...
    # Mutation corrigée : le texte stocké en base prime sur le code (sources 19.0).
    # Le même oracle doit rejeter le mauvais message réellement retourné.
    assert expected in good_code
    mutation = project / 'mutate_message.py'
    mutation.write_text("env['ir.model.constraint'].search([('name', '=', 'lab_rental_check_days_positive')]).write({'message': 'Message incorrect de contre-épreuve.'})\nenv.cr.commit()\n")
    lab.handle(['shell', str(mutation)])
    ...
    mutation.write_text("env['ir.model.constraint'].search([('name', '=', 'lab_rental_check_days_positive')]).write({'message': " + repr(expected) + "})\nenv.cr.commit()\n")
    lab.handle(['shell', str(mutation)])
