...
À cette jointure, rapproche d'abord les critères originaux et les limites
déclarées dans les fragments. Si un résultat obligatoire n'a pas été vérifié,
consigne dans `qa.md` le **critère exact**, la preuve disponible et ce qui manque,
puis enregistre `retry` (ou l'issue d'arrêt prévue). Une nouvelle lecture du code
ne remplace pas l'exécution absente ; inutile de refaire l'audit complet pour
statuer sur cette lacune. Un report proposé par une voie ne modifie pas le contrat.
Sinon, établis la couverture et le verdict global : conserve la portée de chaque
critère, y compris ses différentes conditions ; tous ses résultats obligatoires
doivent être prouvés pour le compter satisfait. Les titres « vert » des voies ne
sont pas des preuves. Les contrôles explicitement hors contrat restent hors du
décompte. N'émet jamais `pass` avec un résultat obligatoire non prouvé.
