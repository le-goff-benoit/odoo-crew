...
À cette jointure, construis d'abord la table de couverture depuis la revue
fonctionnelle, avant le verdict global : conserve le **texte intégral** de chaque
critère et sépare ses résultats attendus s'il en contient plusieurs. Pour chaque
résultat, rattache la preuve d'exécution ou de lecture pertinente et marque
**prouvé / non prouvé / faux**. Les titres « vert » des voies ne sont pas des
preuves. Calcule ensuite la satisfaction : un critère n'est satisfait que si
tous ses résultats obligatoires sont prouvés. Un résultat non prouvé appelle une
reprise (ou l'arrêt prévu si elle est impossible), même lorsqu'une voie le reporte
à la clôture ; n'émet pas `pass` dans ce cas. Les contrôles explicitement hors
contrat restent hors du décompte.
