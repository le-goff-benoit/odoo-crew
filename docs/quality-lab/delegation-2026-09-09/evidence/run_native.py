...
mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod);mod.ROOT=ROOT
folder=BASE/'N04-claude-delegated'
if folder.exists(): raise SystemExit('Dossier déjà utilisé ; aucun rejeu silencieux')
stop=threading.Event();started=time.monotonic()
def monitor():
 positions={}
 with (BASE/'native-timeline.jsonl').open('w') as out:
